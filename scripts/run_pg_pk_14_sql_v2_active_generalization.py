"""PG-PK-14: active SQL probe on an unseen transport/source implementation."""
...
PROTOCOL_ID = "pg-pk-14-sql-v2-active-generalization-v1"
...
REPORT_PATH = ROOT / "research" / "pg_pk_14_sql_v2_active_generalization_v1.json"
MARKDOWN_PATH = ROOT / "research" / "pg_pk_14_sql_v2_active_generalization_v1.md"
PROTOCOL_PATH = ROOT / "research" / "pg_pk_14_sql_v2_active_generalization_protocol_v1.json"
PG09_REPORT_PATH = ROOT / "research" / "pg_pk_09_sql_differential_v1.json"
PRE_FIX_FAILURE_PATH = ROOT / "research" / "pg_pk_14_sql_v2_pre_fix_quarantine_v1.json"
TARGETS = ((8806, "alpha", "sql_v2_alpha"), (8807, "beta", "sql_v2_beta"), (8808, "gamma", "sql_v2_gamma"))
SEEDS = (20310101, 20310107, 20310113)
MAX_REQUESTS = 13
SQL_V2_DATASET_ID = "sql_differential_fixture_v2"
...
def _collect(collector: SqlV2Collector, spec: dict[str, Any]) -> dict[str, Any]:
    return asyncio.run(collector.collect(spec))


def _ledger_row(record: dict[str, Any], *, dataset_id: str, seed: int, accepted: bool, false_positive: bool = False) -> dict[str, Any]:
...
def _run_episode(
    *,
    ...
    server = make_sql_v2_fixture_server(port=port, variant=variant)
    thread = threading.Thread(target=server.serve_forever, name=f"pg14-{port}-{seed}", daemon=True)
    thread.start()
    ...
    try:
        ...
        specs = default_sql_v2_specs(dataset_id=f"{dataset_id}-seed-{seed}", target=target, marker=f"pg14-{variant}-{seed}")
        ...
        for spec in screening[:MAX_REQUESTS]:
            record = _collect(SqlV2Collector(base_url=target, target_instance_id=f"{dataset_id}-seed-{seed}", source_hash=source_hash), spec)
            ...
            pair_id = str((spec.get("pair") or {}).get("pair_id", "control"))
            trace.append({"stage": "screen", "pair_id": pair_id, "variant": (spec.get("pair") or {}).get("variant", "plain"), "decoder_candidate": output.get("candidate_family"), "decoder_abstained": output.get("abstained", True), "posterior": step["posterior"], "shared_router": shared_route, "evidence_hash": record["evidence"]["evidence_hash"]})
            for candidate in by_pair.get(pair_id, [])[1:]:
        ...
        while refinements and len(records) < MAX_REQUESTS:
            ...
            record = _collect(SqlV2Collector(base_url=target, target_instance_id=f"{dataset_id}-seed-{seed}", source_hash=source_hash), selected)
            output = _decode(model, mean, std, threshold, record)
            shared_route = shared_router.inspect(record)
            likelihood = _fuse_shared_route(_decoder_likelihood(output), shared_route)
            likelihood.update({key: likelihood[key] * value for key, value in _projection_likelihood(record).items()})
            step = belief.observe(selected["path"], likelihood, evidence_hash=record["evidence"]["evidence_hash"])
            record["sql_decoder"] = output
            record["shared_router"] = shared_route
            records.append(record)
            pair = selected.get("pair") or {}
            trace.append({"stage": "refine", "pair_id": pair.get("pair_id", ""), "variant": pair.get("variant", ""), "decoder_candidate": output.get("candidate_family"), "decoder_abstained": output.get("abstained", True), "posterior": step["posterior"], "shared_router": shared_route, "evidence_hash": record["evidence"]["evidence_hash"]})
        ...
        for pair_id, pair_records in sorted(grouped_records.items()):
            ...
            # Active fallback schedules the typed SQL oracle even if the
            # decoder abstains.  This is an evaluation lane, not a positive
            # model output.
            candidate_records = [dict(record, candidate_family="injection") for record in pair_records]
            result = revalidate_sql_pair(candidate_records, authorized_source_hash=source_hash, oracle_name=SQL_V2_ORACLE)
            ...
            if result.get("accepted"):
                ledger.extend(_ledger_row(record, dataset_id=SQL_V2_DATASET_ID, seed=seed, accepted=True) for record in pair_records)
            else:
                for record, prediction in zip(pair_records, predictions):
                    ...
                    ledger.append(_ledger_row(record, dataset_id=SQL_V2_DATASET_ID, seed=seed, accepted=False, false_positive=model_candidate))
        stats = {
...
def main() -> None:
    if not DECODER_CHECKPOINT.exists() or not SHARED_CHECKPOINT.exists() or not PG09_REPORT_PATH.exists():
        raise FileNotFoundError("PG-PK-14 requires SQL/shared checkpoints and a fresh PG-PK-09 report")
    ...
    v1_report = json.loads(PG09_REPORT_PATH.read_text(encoding="utf-8"))
    v1_ledger = [dict(row) for row in (v1_report.get("promotion_ledger") or [])]
    combined_ledger = v1_ledger + ledger
    ...
    report = {
        ...
        "schema_version": "sift-pg-pk-14-sql-v2-active-generalization-report-v1",
    ...
    MARKDOWN_PATH.write_text(
        ...
        f"样本请求：{report['request_count']}/{report['static_request_count']}；target：{len(TARGETS)}；seed：{len(SEEDS)}；complete pair：{report['complete_pair_count']}。\n\n"
        f"typed oracle pair：{report['oracle_revalidated_pair_count']}；decoder 直接 pair：{report['decoder_pair_candidate_count']}；abstain 后 fallback 找回：{report['fallback_oracle_pair_count']}；decoder abstain：{report['decoder_abstain_count']}；false positive ledger：{report['false_positive_ledger_row_count']}。\n\n"
        f"v2 本地门：`{report['local_source_promotion']['status']}`；跨 v1+v2 source 门：`{report['promotion']['status']}`；source hash：{report['provenance']['source_count']}（同一 source hash 的 variant 不计作独立数据集）。\n\n"
    ...
    PROTOCOL_PATH.write_text(json.dumps({
        "protocol_id": PROTOCOL_ID,
        "surfaces": ["/lookup?channel=...", "renamed_abstract_fragments", "changed_json_shapes"],
        ...
        "positive_gate": {"oracle": SQL_V2_ORACLE, "pair": ["plain", "url_percent"], "source_hash": True, "evidence_hash": True},
        "active_policy": {"decoder_abstain_fallback": True, "shared_router_positive_authority": False, "oracle_only_positive": True},
        "promotion": {"local_v2": report["local_source_promotion"], "cross_source_v1_v2": report["promotion"]},
        "preserved_pre_fix_failure": str(PRE_FIX_FAILURE_PATH.relative_to(ROOT)) if PRE_FIX_FAILURE_PATH.exists() else None,
        "safety": report["safety"],
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"protocol_id": PROTOCOL_ID, "status": report["status"], "request_count": report["request_count"], "complete_pair_count": report["complete_pair_count"], "oracle_revalidated_pair_count": report["oracle_revalidated_pair_count"], "decoder_pair_candidate_count": report["decoder_pair_candidate_count"], "fallback_oracle_pair_count": report["fallback_oracle_pair_count"], "decoder_abstain_count": report["decoder_abstain_count"], "false_positive_ledger_row_count": report["false_positive_ledger_row_count"], "report": report["report_path"], "markdown": str(MARKDOWN_PATH.relative_to(ROOT))}, ensure_ascii=False, indent=2))
