"""Collect PG-36 delayed/ambiguous maze traces on fresh loopback targets."""
...
from app.pg36_independent_maze_fixture import LAYOUTS, PHASES, SURFACE_SPECS, make_pg36_server  # noqa: E402
...
CATALOG_OUTPUT = ROOT / "research" / "pg36_independent_maze_catalog_v1.json"
TRACE_OUTPUT = ROOT / "research" / "pg36_independent_maze_trace_v1.json"
SEEDS = (361, 367, 373)
TARGET_PORT = 31936
ORACLE_CONTRACT_SHA256 = hashlib.sha256(b"pg36-independent-maze-typed-oracle-v1").hexdigest()
ROLE_FAMILIES = {
    "train": ["xss", "injection"],
...
class _FreshTarget:
    def __init__(self, implementation: str, port: int = TARGET_PORT) -> None:
        ...
        self.port = int(port)
        self.server = make_pg36_server(self.port, self.implementation)
    ...
    def __enter__(self) -> httpx.Client:
        ...
        while time.monotonic() < deadline:
            ...
            raise RuntimeError("PG-36 fixture did not start")
...
def _call(client: httpx.Client, implementation: str, surface: str, method: str, phase: str, *, positive: bool) -> httpx.Response:
    ...
    route = _route(implementation, surface)
    values = {layout["slot_key"]: surface, layout["probe_key"]: _wire(token), layout["phase_key"]: phase}
    if method == "GET":
        return client.get(route, params=values)
    if method != "POST":
        raise ValueError("PG-36 collector permits only GET and POST")
...
def _oracle(implementation: str, surface: str, method: str, phase: str, parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "oracle_id": f"pg36-independent-{spec['family']}-oracle-v1",
        ...
        "signals": {"implementation": implementation, "method": method, "phase": phase, **safe_keys},
        "safety": {
...
def _source(implementation: str, collector_hash: str, fixture_hash: str) -> dict[str, Any]:
    return {
        "target_id": "pg36_independent_maze_fixture",
        "app_family": "standalone_python_http_maze_v1",
        "source_id": f"pg36-independent-source-{implementation}",
        "source_type": "in_repo_synthetic",
        "origin_ref": "app/pg36_independent_maze_fixture.py:/<implementation>/<surface-id>",
        "license": "in-repo-synthetic",
        "authorization": "workspace_local_only",
        "loopback_scope": {"scheme": "http", "host": "127.0.0.1", "port": TARGET_PORT},
        "fixture_source_sha256": sha256_json({"fixture": fixture_hash, "implementation": implementation}),
        "collector_sha256": collector_hash,
        "reset_adapter_sha256": hashlib.sha256(f"pg36-independent-fresh-reset:{implementation}".encode()).hexdigest(),
        "oracle_contract_sha256": ORACLE_CONTRACT_SHA256,
        "read_only": True,
        "external_network": False,
    }
...
def _reset(source: dict[str, Any], implementation: str, surface: str, seed: int, method: str, phase: str, role: str, baseline: dict[str, Any]) -> dict[str, Any]:
    suffix = f"{implementation}-{surface}-s{seed}-{method.casefold()}-{phase}-{role}"
    result = {
        "reset_id": f"pg36-reset-{suffix}",
        "kind": "fresh_pg36_http_server",
        "target_instance_id": f"pg36-target-{suffix}",
        "state_epoch": f"pg36-epoch-{suffix}",
...
def _manifest(implementation: str, surface: str, seed: int, method: str, phase: str, role: str) -> dict[str, Any]:
    spec = SURFACE_SPECS[surface]
    layout = LAYOUTS[implementation]
    probe_kind = "http_canary" if phase == "timeout" else spec["probe_kind"]
    safe_basis = {"fixture": "pg36", "implementation": implementation, "surface": surface, "seed": seed, "method": method, "phase": phase, "role": role}
    return {
        "manifest_id": f"pg36-{implementation}-{surface}-s{seed}-{method.casefold()}-{phase}-{role}",
        "payload_sha256": sha256_json({"basis": safe_basis, "probe_kind": probe_kind}),
        "probe_ref": f"pg36-safe-{spec['family']}-{phase}",
        "probe_kind": probe_kind,
        "route_template_id": f"pg36-{implementation}-{surface}",
        "method": method,
        "placement": "query" if method == "GET" else "form",
        "encoding_chain": ["identity"],
        "encoding_depth": 0,
        "marker_sha256": hashlib.sha256(f"pg36-safe-marker:{implementation}:{surface}:{seed}".encode()).hexdigest(),
        "max_bytes": 256,
        "form_field_names": [layout["slot_key"], layout["probe_key"], layout["phase_key"]] if method == "POST" else [],
...
def _rule_ir(implementation: str, surface: str, method: str, phase: str) -> dict[str, Any]:
    return {
        "rule_key": f"{SURFACE_SPECS[surface]['family']}.pg36-{implementation}-{surface}.{method.casefold()}.{phase}",
        "grammar_version": "rule-ir-v1",
        "family_candidate": SURFACE_SPECS[surface]["family"],
...
def main() -> int:
    ...
    for implementation in LAYOUTS:
        ...
        for surface, spec in SURFACE_SPECS.items():
            ...
            for seed in SEEDS:
                episode_id = f"pg36-episode-{implementation}-{surface}-s{seed}"
                ...
                for method in ("GET", "POST"):
                    for phase in PHASES:
                        ...
                        with _FreshTarget(implementation) as client:
                            candidate_response = _call(client, implementation, surface, method, phase, positive=role != "negative_control")
                        ...
                        control_id = f"pg36-{implementation}-{surface}-s{seed}-{method.casefold()}-{phase}-control"
                        candidate_id = f"pg36-{implementation}-{surface}-s{seed}-{method.casefold()}-{phase}-candidate"
                        control = collector.collect(sample_id=control_id, sample_role="negative_control", sampling_seed=seed, reset=_reset(collector.source, implementation, surface, seed, method, phase, role + "-control", baseline), payload_manifest=_manifest(implementation, surface, seed, method, phase, "control"), response_projection=control_projection, oracle_projection=control_oracle, rule_ir=_rule_ir(implementation, surface, method, phase))
                        candidate = collector.collect(sample_id=candidate_id, sample_role="candidate" if candidate_positive else "negative_control", sampling_seed=seed, reset=_reset(collector.source, implementation, surface, seed, method, phase, role + "-candidate", baseline), payload_manifest=_manifest(implementation, surface, seed, method, phase, "candidate"), response_projection=candidate_projection, oracle_projection=candidate_oracle, rule_ir=_rule_ir(implementation, surface, method, phase), negative_control=({"control_sample_id": control["sample_id"], "control_evidence_hash": control["evidence"]["evidence_hash"], "intervention": "typed-class-vs-normal-control", "verdict": "confirmed_negative", "same_source": True, "same_surface": True} if candidate_positive else None))
                        for row, sample_role, pair_role in ((control, "negative_control", "control"), (candidate, "candidate" if candidate_positive else "negative_control", "candidate")):
                            row.update({"dataset_role": role, "implementation": implementation, "surface_id": surface, "family": spec["family"], "method": method, "phase": phase, "sample_role": sample_role, "pair_role": pair_role, "maze_stage": "delayed_confirm" if phase == "confirm" else "ambiguous_screen" if phase == "screen" else "negative_control"})
                        ...
                        for row, pair_id, action in ((control, None, "replay_candidate"), (candidate, control["sample_id"] if candidate_positive else None, "confirm_same_surface" if phase == "screen" else "stop_episode" if candidate_positive else "next_probe")):
                            # sample_role is intentionally negative_control for
                            # both sides of a negative pair; pair_role keeps
                            # trace identity injective without exposing input.
        ...
        variant_catalog = build_catalog(f"pg36-independent-{implementation}-catalog", collector.source, implementation_records)
    ...
    for role, families in ROLE_FAMILIES.items():
        for seed in SEEDS:
            ...
            summary = {"sample_id": f"pg36-test-{role}-s{seed}", "dataset_id": f"pg36-{role}-s{seed}-v1", "source_id": f"pg36-role-source-{role}-s{seed}", "source_hash": sha256_json(source_hashes), "target_instance_ids": targets, "family_set": families, "sampling_seed": seed, "role": role, "sample_count": len(rows), "unique_sample_count": len(sample_ids), "denominator": len(rows), "positive_count": sum(int(row["oracle_projection"]["positive"]) for row in rows), "negative_count": sum(int(not row["oracle_projection"]["positive"]) for row in rows), "abstain_count": 0, "method_set": sorted({row["method"] for row in rows}), "phase_set": sorted({row["phase"] for row in rows}), "source_count": len(source_hashes), "dataset_manifest_sha256": sha256_json({"role": role, "seed": seed, "samples": sample_ids}), "split_manifest_sha256": sha256_json({"role": role, "seed": seed, "targets": targets, "families": families}), "probe_sha256": sha256_json([row["payload_manifest"]["payload_sha256"] for row in rows]), "oracle_contract_sha256": ORACLE_CONTRACT_SHA256, "checkpoint_sha256": sha256_json("pending-model-run"), "metrics_status": "pending_model_run", "metrics": {"typed_recall": 0.0, "precision": 0.0, "false_positive_rate": 0.0, "abstain_precision": 0.0, "ece": 0.0, "median_queries": 0.0}}
            summary["evidence_hash"] = sha256_json(summary)
            dataset_tests.append(summary)

    catalog = {"schema_version": "pg-pk-36-independent-maze-catalog-v1", "catalog_id": "pg36-independent-maze-v1", "purpose": "independent local GET/POST delayed/ambiguous typed replay with deterministic error and timeout-like negative controls", "runtime_replay": True, "independent_target_implementation": True, "evaluation_only": True, "training_eligible": False, "training_artifact_generated": False, "model_evaluation_completed": False, "methods": ["GET", "POST"], "phases": list(PHASES), "seeds": list(SEEDS), "implementations": list(LAYOUTS), "sources": sources, "source_catalogs": source_catalogs, "samples": all_records, "dataset_tests": dataset_tests, "trace_dataset": str(TRACE_OUTPUT.relative_to(ROOT)), "trace_episode_count": len(episode_reports), "accepted_evaluation_episode_count": sum(int(item["status"] == "accepted_evaluation") for item in episode_reports), "typed_positive_count": sum(int(row["oracle_projection"]["positive"]) for row in all_records), "negative_control_count": sum(int(not row["oracle_projection"]["positive"]) for row in all_records), "delayed_confirm_count": sum(int(row["phase"] == "confirm") for row in all_records), "ambiguous_screen_count": sum(int(row["phase"] == "screen") for row in all_records), "deterministic_error_count": sum(int(row["phase"] == "error") for row in all_records), "deterministic_timeout_count": sum(int(row["phase"] == "timeout") for row in all_records), "fresh_reset_count": len(all_records), "source_count": len({row["source_sha256"] for row in all_records}), "target_instance_count": len({row["target_instance_id"] for row in all_records}), "raw_probe_strings_stored": False, "raw_response_bodies_stored": False, "external_network": False, "authorization": "workspace_local_only", "family_policy": ROLE_FAMILIES, "manifest_sha256": sha256_json({"samples": [row["evidence"]["evidence_hash"] for row in all_records], "dataset_tests": dataset_tests})}
    trace_dataset = {"schema_version": "pg-pk-36-independent-maze-trace-v1", "purpose": "delayed confirm/ambiguous screen active-probe trace", "evaluation_only": True, "training_eligible": False, "independent_target_implementation": True, "raw_probe_strings_stored": False, "raw_response_bodies_stored": False, "methods": ["GET", "POST"], "phases": list(PHASES), "episode_count": len(episode_reports), "accepted_evaluation_episode_count": sum(int(item["status"] == "accepted_evaluation") for item in episode_reports), "episodes": episode_reports, "steps": trace_steps, "catalog_manifest_sha256": catalog["manifest_sha256"], "trace_manifest_sha256": trace_sha256_json([step["trace_sha256"] for step in trace_steps])}
    ...
    print(json.dumps({"catalog": str(CATALOG_OUTPUT.relative_to(ROOT)), "trace_dataset": str(TRACE_OUTPUT.relative_to(ROOT)), "sample_count": len(all_records), "typed_positive_count": catalog["typed_positive_count"], "negative_control_count": catalog["negative_control_count"], "delayed_confirm_count": catalog["delayed_confirm_count"], "ambiguous_screen_count": catalog["ambiguous_screen_count"], "error_count": catalog["deterministic_error_count"], "timeout_count": catalog["deterministic_timeout_count"], "source_count": catalog["source_count"], "episode_count": len(episode_reports), "accepted_evaluation_episodes": catalog["accepted_evaluation_episode_count"], "training_eligible": False}, ensure_ascii=False, indent=2))
    return 0
