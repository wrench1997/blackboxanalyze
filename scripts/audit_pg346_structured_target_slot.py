"""Read-only audit for the PG-346 structured target-slot candidate report."""
...
SCHEMA_VERSION = "pg346-structured-target-slot-audit-v1"
...
def audit_report(report: Mapping[str, Any], *, report_file_sha256: str | None = None) -> dict[str, Any]:
    ...
    # The implementation holdout intentionally has no safe_to_send=1 rows;
    # positive recall is therefore not applicable, while negative abstention
    # is still mandatory and is checked above.
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit PG-346 structured target-slot candidate report")
