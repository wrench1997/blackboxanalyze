...
const cases: LogicCase[] = [
  ...
  {
    id: "concurrency-lock",
    backendCaseRef: "purchase_concurrency_lock",
    group: "交易 / 并发",
    title: "订单并发提交",
    short: "同一版本只能提交一次",
    method: "POST",
    surface: "order_commit_lock",
    invariant: "order_version → one_commit",
    preconditions: ["order_version=observed", "commit_state=pending", "fresh_episode=true"],
    counterfactual: "并发/重放只能有一个 commit effect。",
    failure: "commit=accepted_twice · version_binding=missing",
    decision: "REPAIR",
    action: "引入 version / idempotency 观察，再做一次受控 replay。",
    negative: "reference 应返回 duplicate/locked shape；不写订单。",
    replay: "每角色独立 fresh order；不跨角色共享版本。",
    oracle: "commit_version_shape + effect_delta_zero",
    boundary: "只在内存 disposable canary 观察锁竞争形状，不执行真实交易。",
    tokens: ["transport=POST", "state=pending", "binding=order_version", "concurrency=single_commit", "oracle=commit_shape"],
    stages: ["baseline", "读取版本边界", "candidate/replay", "reference", "negative", "fresh reset"],
  },
  {
    id: "coupon-ledger",
