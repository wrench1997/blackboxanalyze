...
- 本轮 PG388/PG389/typed 专项回归=`86 passed, 5 warnings`；全仓收集到 `1821` tests，但 240 秒全量执行窗口超时且未报告单测失败，已终止残留的本轮 pytest 子进程；后续以专项回归和分组全量执行为准，不把超时误写成通过。
- GitHub 交接文档已同步 typed 版本：`docs/GITHUB_DEMO_RELEASE.md` SHA=`f6f4fee367a3ba0384eaac06f03e6557a02e6d37a54835882ce38ba83df685b0`，`docs/PG388_DEMO_RUNBOOK.md` SHA=`21b4d761cd579c804c9cdc41824ce234267a6a7783ea69ab8af6e54670ca2d7b`；交接明确 66-case taxonomy、120 条 evaluator-diagnostic typed rows 和正确的 `/pg388-api` 健康检查。
- 运行态回归补充：`/pg385`、`/pg388`、`/pg389`、`/pg388-api/health`、`/pg388-api/api/manifest` 均 HTTP 200；PG-385/PG-377/PG-379/PG-388/PG-389 相关分组测试=`167 passed, 4 warnings`。全仓 1821 项仍需分组执行，不能用超时结果替代。
...
### 2026-08-10：PG-388 抽象信息保真审计接入展示

- 新增只读审计 `scripts/audit_pg388_logic_information_preservation.py`（SHA=`0a442d31008dd082321f7ca66b3e57b0f49a3f84acf446b1484fb88a5463733d`）及测试 `tests/test_pg388_logic_information_preservation.py`（SHA=`438c179ef2b7de5d86aa3e3f580ef7fc1507553ecf68d887711849f6cdf8faea`）。审计只输出 bounded counts/长度/熵/容量/哈希，不输出 rows、token 序列、payload、wire、响应体或 evaluator 答案。
- 报告 `research/pg388_logic_information_preservation_audit_v1.json`（SHA=`45be804aace61f29d80a620a2f6c7e37a6bbceddb46530e197efaf0289f276b4`）状态=`blocked_information_gate`：两实现共 `280` 条 holdout row，context unique ratio=`0.007143`、target unique ratio=`0.025`，7 个 presence 轴全部 observed 但 entropy=`0`；context max=`2261`、required window=`2261`，且 train split 缺失。`training_eligible=0`、promotion/training/memory/payload/vulnerability 全部关闭。
- `scripts/project_pg388_logic_frontend_summary.py`（SHA=`655857072aed354b8cdee1e0e415822e0ad8ce42a8b1cc8c14986419235025fb`）、`frontend/components/pg388-logic-lab.tsx`（SHA=`c537a56e7865e504883d4b0ad391a6f5603fd2bd22ee795d40b7e0d50c60dfd5`）和 `tests/test_pg388_logic_frontend_projection.py`（SHA=`2fe4732b514c87caaf4752c186e1b262962d751de9d8ddda4cf784e317d7b1c4`）已接入 `INFORMATION PRESERVATION` 卡片；摘要 `frontend/public/research/pg388_logic_rule_ir_frontend_summary_v1.json`（SHA=`b99a501789d7fa6f1c0f06c9f88d8435ee433356810d84ad8bcf8a827515fdca`）只显示聚合诊断，`information_preservation_audit=false`。
- 资产清单已登记上述 3 个新审计文件与更新后的摘要/组件，校验=`asset_count=86`、`missing_optional=[]`；相关 focused 回归=`4 passed`，脚本 py_compile 通过。该审计说明当前数据存在表示塌缩/缺训练 split，不能据此申请 A800 或宣称通用漏洞/payload 能力。
- 随后用已完成的 `.next/standalone` 做了 `network=none` 的离线增量刷新（未下载依赖、未接触外网），前端运行镜像更新为 `sha256:151abd41a9bab35964ad119b8de1ddbcd2b9c10c23b566daddb2ef4fe1c31925`；`GET /pg388=200`，摘要静态资源可读取且含 `information_preservation`/`0.007143`。临时 Dockerfile/ignore 文件已删除，工作树保持干净。

### 2026-08-10：PG-388 composition train/holdout 合同复核

- `scripts/audit_pg388_logic_information_preservation.py`（当前 SHA=`2ed5aab660c270be524f9183c14fa1e096d3addf21a57427880aeab6008fb486`）现在同时读取既有 composition dataset（SHA=`ae9461fa84062ab2d9a96dbd3450ff83932e4f2f79d7ba7ae51703faeccef26`），只投影 split/长度/唯一率/跨 split overlap/抽象轴覆盖/合同布尔值；不复制 dataset rows 或 token 值。
- composition dataset 有 `840` 条抽象行（train/implementation_holdout=`420/420`），train context unique ratio=`0.333333`，但 target 跨 split overlap=`140`；7 个整页信息轴的 token coverage=`0/7`，row-bound typed evidence、fresh reset、operator review 均未满足，`training_eligible=0`。因此信息门仍=`blocked_information_gate`，旧 strict source-row 采集仍缺 train split；禁止将该 composition 诊断行重标为 gold/train。
- `/pg388` 信息卡新增 composition train/holdout 合同摘要；摘要 SHA=`1a4939879f601f2b7f71148d670686681efe60a763669cad1c89fb608fb437c6`，组件 SHA=`a00af1ef2794ed56e78d976758e8eaa2a24065de1f7e1b0dabe70a786396c718`，投影脚本 SHA=`e6db221acdcb98b7ef2dc17abe5e02fc38aa279c880db58383347eca7ce9aaf0`，前端测试 SHA=`45a6ed4601d7642e01edb3e8a56f29236ac1f9b1596cdd43a04b5e49b6aa3865`；相关 focused=`4 passed`、Next build 通过，资产校验=`asset_count=86`。
- 组合摘要更新后再次以 `network=none` 离线刷新前端，当前运行镜像=`sha256:df56f2c202d7ceebde7e768dd7ea65dc09790c3745938e555fc180a4ebd76c16`；`GET /pg388=200`，静态摘要确认包含 composition train/holdout、context unique ratio 和 target overlap 聚合字段。

### 2026-08-10：PG-388 多表面整页 source-row 诊断生成器

- 新增纯进程内生成器 `scripts/build_pg388_logic_surface_source_rows.py`（SHA=`55bfc98f5b98b769d24a028b2ce85b123845c2af7ff0d6d6ff0a74952c135722`）及 focused 测试 `tests/test_pg388_logic_surface_source_rows.py`（SHA=`da3b79d650512c250e6857ececb9e0d77185cfe588e954c475c11d9333f1554d`）。它只把短暂 HTML/JS 表面交给已审阅 PG-377 adapter，落盘前丢弃 markup/source；不启动 Docker、GPU、网络，不生成 wire/payload。
- 产物 `research/pg388_logic_surface_source_rows_v1.json`（SHA=`bb59a0fe03e033669cd0a108af9bec6abcaf6075e87cadaa6c7b0e31eb5ec480`）含 `144` 条抽象行：2 个不同实现表面（C=GET/train、D=POST/implementation_holdout）× 3 seed × 6 case × candidate/reference/negative/replay；`strict_valid=136`、incomplete=`8`、typed/fresh=`144/144`、negative violation=`0`、raw marker hits=`0`。其中 8 条 JSON/API 表面显式保留 document/navigation/JavaScript `not_observed` 并强制 ASK，不伪造完整页。
- 该数据用于信息保真/JS 语义投影诊断，不是 live evaluator gold：`in_process_fixture_only=true`、`operator_reviewed=false`、`training_eligible=0`、promotion/training/memory/payload/vulnerability 全部关闭；即使 split 计数为 train/holdout=`72/72`，也禁止直接训练或长期记忆晋级。资产清单已由 `86` 更新为 `89`，校验 `missing_optional=[]` 通过。

### 2026-08-10：PG-388 多表面数据接入信息审计与前端

- `scripts/audit_pg388_logic_information_preservation.py`（SHA=`05b4691d79b26d8eaf9479c28c9b760742332050a4ad91d9da5642441d8e1c1d`）新增 `--surface-rows`，只输出多表面数据的 split、实现数、context/target 唯一率、7 轴 observed/熵、抽象 prefix value diversity、raw marker 计数和 source contract；报告不包含 rows、tokens、JS 源码、payload 或 evaluator 答案。
- 当前信息审计报告=`research/pg388_logic_information_preservation_audit_v1.json`（SHA=`912e21e9bbd178a5f40accf0d5bfa825672da0dc799ad1e94b986ad89b18adda`）。surface projection：`144` rows、train/implementation_holdout=`72/72`、两实现、context unique ratio=`0.166667/0.222222`、target=`0.055556/0.069444`、cross-split context/target overlap=`0/0`；document/navigation/JavaScript 三轴熵=`0.309543`，request/response/failure/belief presence 仍零熵，但抽象 value prefix `47/194` 非常量、最大熵=`3.821928` bits。8 条 API 表面显式 `not_observed`，operator review/training contract 仍缺失，整体 status 仍=`blocked_information_gate`。
- `/pg388` 信息卡新增 `SURFACE ROWS` 与 `VALUE SHAPES` 聚合项；摘要=`frontend/public/research/pg388_logic_rule_ir_frontend_summary_v1.json`（SHA=`f9a7620aeaa14d4a1e032cf8a51d2507d44656d4561b4f2fd3c66e3a41ded9f9`），投影脚本 SHA=`d930c73a5893dabe4644b94f48012f362940e4000960a280efc4143ede321e67`，组件 SHA=`7e1b748c67f1f49c9424b8ef71ff9be1f9720502a69758b7ff865736815b986e`。
- 前端已用本轮 `.next/standalone` 做 `network=none` 增量刷新，当前 `pg388-frontend:local` image=`sha256:dd429188ebf13ec1c6fe3a59af26d019e20f1ec80ad06a9743f9e6c0ff245305`；重启后 `/pg388` 与摘要接口 HTTP 200，摘要包含 `value_diversity_nonconstant_prefix_count`。训练/holdout/晋级保持关闭。
- 相关 tests：`tests/test_pg388_logic_information_preservation.py` SHA=`4f0d015e4a450b18436a0dff2c2b16fd4b38ee83ad76e964e32e3a07d99a0f2b`、`tests/test_pg388_logic_frontend_projection.py` SHA=`3fbf95440ced0a41639fcbd9084482a9e0cdff26e07b1235fb63a7c2e7feebd0`；专项回归与资产校验需重新执行，训练/晋级保持关闭。

### 2026-08-10：PG-388 surface candidate 交接计划

- 新增只读计划器 `scripts/plan_pg388_logic_surface_candidate.py`（SHA=`3c7c63f69a8ebe9b142dc9f4ebd8c96949b131f9264b42b74dc0f06d9c736e93`）及测试 `tests/test_pg388_logic_surface_candidate.py`（SHA=`97c60d406af2372d963472fde401a8ce3970e97544f284e829473519a2a032b0`）。计划报告 `research/pg388_logic_surface_candidate_plan_v1.json`（SHA=`81ca66e73b8337f29985b49d3f071a11839a81590f25ee15fa80cad5d3004318`）状态=`blocked_surface_source_contract`：144 条 source-row wrapper、train/implementation_holdout=`72/72`、跨 split context/target overlap=`0/0`，但不完整行=`8`、holdout train-only vocabulary gap=`43`、operator review 缺失、training eligible=`0`；optimizer/GPU/Docker/network/checkpoint 全部 false。
- 计划器不输出 rows、token 序列、payload、wire、响应体或 evaluator 答案；focused 回归 `10 passed`，资产清单校验 `asset_count=92`。公开 GitHub 只交付代码/抽象数据/审计报告；约 51.3 GiB 的 `artifacts/` 权重继续受 `.gitignore` 保护，不上传公开仓库。
- 交接时仅将一份抽象 PG-384 Rule-IR composition checkpoint `artifacts/pg384-binding-composition-a800/pg375_seed_38101.pt` 作为可选 Git LFS 资产登记，大小=`589728980`、SHA=`8fa769c3dbc91c1a803733ac56e038847e29f5477655c7e6d5077cb7ab879b4d`；PG-385/386 过滤/解码权重仍不公开，所有 candidate/promotion/training/vulnerability 标志保持关闭。
- 用户随后明确授权公开最新过滤/解码候选；新增 Git LFS 资产：PG-385 filter SFT `artifacts/pg385-filter-sft/pg370_seed_38003.pt`（572179304 bytes，SHA=`f6265ecd19acb1d53c8debba5cb9ca8002bd9b2edc3e0c183dee75b8fddec98f`）与 PG-386 fixture decoder head `artifacts/pg386-fixture-payload-decoder/pg386_payload_head_seed_38603.pt`（251794 bytes，SHA=`ea57370656898b03b3988ddbe2fa94c69f126baf7dacd3e7451612e977d13827`）。三份公开权重均为 candidate-only、本地 canary 研究证据，不得宣传为通用 WAF 绕过/任意 payload；其余约 50 GiB 历史权重继续不上传。
