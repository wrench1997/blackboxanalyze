...
- 新增 `fixtures/pg388/logic_lab_b.py`（SHA=`4551c218c0ed2dfbd4f3cf565bad53afb76fc4d9e8aaec10bf255ba38c67ba57`）。B 使用独立的 transition/effect 表和实现模块，但继续只接受 `case_ref/role/phase` 枚举；`safe_to_send=false`、无任意值、无持久化、无外网、无真实业务写入。
...
- B 静态 WSGI 合同测试 `tests/test_pg388_logic_lab_b.py` SHA=`83e97b71cd0c7c9b9e880891d747cf34d773f7b6d060e9ca5a09a37e6e0f77f3`，与部署合同回归共 `13 passed`。当前 B `image_attested=false`、`source_rows_emitted=false`、`training_eligible=0`；不得把静态差异表伪装成 live typed 或训练样本。更新后 rules SHA=`adadba5ef706b09dcb88974e8dbc6b443d7dd544c68a266f5fa5f439bb570a8a`，asset manifest SHA=`c09a0ac834151a95db3df3718a4620c3b34750ac1e1805fcdaaf936321e1636d`。
