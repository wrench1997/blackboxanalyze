...
- 新增纯进程内生成器 `scripts/build_pg388_logic_surface_source_rows.py`（SHA=`11116f986d71a1ecea7c8c5904b53fc4cc2c4cd7adb93a3fa5fd020071f7bb56`）及 focused 测试 `tests/test_pg388_logic_surface_source_rows.py`（SHA=`7be1a57501ed055b0d0e44ffa2992f688ce5fc50738e8bab4fd412c5160ab78f`）。它只把短暂 HTML/JS 表面交给已审阅 PG-377 adapter，落盘前丢弃 markup/source；不启动 Docker、GPU、网络，不生成 wire/payload。
- 产物 `research/pg388_logic_surface_source_rows_v1.json`（SHA=`e4dfe4d2dd44c747a6579d52fac40524cd9cfa52837c72b0b486ad32e8005f97`）含 `144` 条抽象行：2 个不同实现表面（C=GET/train、D=POST/implementation_holdout）× 3 seed × 6 case × candidate/reference/negative/replay；`strict_valid=144`、typed/fresh=`144/144`、negative violation=`0`、raw marker hits=`0`，每行 7 轴/107-field manifest 将 observed 与 absent 分开。
...
- `scripts/audit_pg388_logic_information_preservation.py`（SHA=`ac296b8f4fd2ab563c1301f9226825f58a8498dc8ba951cded23432294c2ec09`）新增 `--surface-rows`，只输出多表面数据的 split、实现数、context/target 唯一率、7 轴 observed/熵、raw marker 计数和 source contract；报告不包含 rows、tokens、JS 源码、payload 或 evaluator 答案。
- 当前信息审计报告=`research/pg388_logic_information_preservation_audit_v1.json`（SHA=`961471999c7ce62d6bc7e300cf144794a447ea2623110dbead783dcf51b7abfa`）。新增 surface projection：`144` rows、train/implementation_holdout=`72/72`、两实现、context unique ratio=`0.166667`、target unique ratio=`0.055556`、cross-split context/target overlap=`0/0`、7/7 轴 observed 但零熵；operator review/training contract 仍缺失，整体 status 仍=`blocked_information_gate`。
- `/pg388` 信息卡新增 `SURFACE ROWS` 聚合项，只显示 `72/72` split、context unique ratio、7 轴覆盖；摘要=`frontend/public/research/pg388_logic_rule_ir_frontend_summary_v1.json`（SHA=`6d0747edcee56f1cd58dc0a34db187a442f296f44b51927a30469ef1075b3ece`），投影脚本 SHA=`e7b19b20af2308a4d462a9c00f1ea839b874aa4706ecf44b84f600e0e74b9b4d`，组件 SHA=`66aa42d6a96359f3ddf4d2148da91f4073e4e374bc020310a66a164445a7a2d3`。
- 前端完成 `network=none` standalone 增量刷新，当前 `pg388-frontend:local` image=`sha256:f7969e9b7f9dde34ac5faca64b0cb1b00a13dd7b44334eaf5e7f48451b29a51e`；`/pg388` 与静态摘要接口均 HTTP 200，后端保持运行，未启动 holdout 或训练。
- 相关 tests：`tests/test_pg388_logic_information_preservation.py` SHA=`4f0d015e4a450b18436a0dff2c2b16fd4b38ee83ad76e964e32e3a07d99a0f2b`、`tests/test_pg388_logic_frontend_projection.py` SHA=`6f391c19bb85f45f6c74c81e47a14857a2be1241f75064a43b3a839805c32342`；专项回归与资产校验需重新执行，训练/晋级保持关闭。
