...
- 资产清单当前 SHA=`2e8fe0fad57a2082f46a6068527ac734be41bb23aad0082c72d7d2e1aac72e21`，校验 `asset_count=57` 通过。尝试用 `network=none` 重建现有前端镜像时因 Dockerfile 的 `npm ci` 超时，未重启当前 `localhost:3000` 容器；现有容器仍 HTTP 200 但尚未显示这次 B projection。重建/更新 live frontend 是下一条部署动作，不得把源代码 build 当作已部署证据。
