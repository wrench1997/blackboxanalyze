"""PG-119 local GET/POST replay bridge for the metadata-transition slot."""
...
from .pg119_metadata_rule_ir_decoder import canonical_model_input
from .pg119_metadata_training_target import SURFACES as TARGET_SURFACES
from .trace_aligned_dataset import evaluate_episode, sha256_json, validate_trace_step


SCHEMA_VERSION = "pg119-metadata-replay-v1"
EVIDENCE_SCHEMA_VERSION = "pg119-metadata-bridge-evidence-v1"
ORACLE_CONTRACT = "pg119-metadata-transition-typed-oracle-v1"
ENCODING_CHAIN = ["unicode_escape", "html_entity", "url_percent"]
...
def _verify_hash(value: Mapping[str, Any]) -> str:
    ...
    if len(declared) != 64 or any(char not in "0123456789abcdef" for char in declared):
        raise ValueError("PG-119 evidence hash is invalid")
    body = dict(value)
    body.pop("evidence_hash", None)
    if sha256_json(body) != declared:
        raise ValueError("PG-119 evidence hash mismatch")
    return declared


def _payload(surface: Surface, candidate: bool, marker: str) -> dict[str, Any]:
    if surface.kind == "metadata":
        return {"metadata_mode": "alternate" if candidate else "baseline"}
...
def _oracle(surface: Surface, response: Mapping[str, Any]) -> tuple[dict[str, Any], bool, bool, str]:
    ...
    if not isinstance(target_evidence, Mapping):
        raise ValueError("PG-119 target omitted evidence")
    target_hash = _verify_hash(target_evidence)
    evaluator = target_evidence.get("evaluator")
    if not isinstance(evaluator, Mapping):
        raise ValueError("PG-119 target omitted evaluator")
    ...
    projection = {
        "modality": "metadata_typed_differential" if available else "metadata_untyped_signal",
        "positive": positive,
        "positive_authority": positive,
        "candidate_signal": candidate,
        "observed_atoms": ["metadata_transition"] if positive else ["candidate_signal_observed"] if candidate else ["no_effect"],
...
async def _action(client: httpx.AsyncClient, surface: Surface, *, seed: int, method: str, role: str, marker: str, state: BspV3State) -> dict[str, Any]:
    ...
    if health.status_code != 200 or health.json().get("implementation") != "pg119-metadata-independent-target":
        raise RuntimeError("PG-119 target identity failed")
    reset_response = await client.post("/v7/reset")
    reset = reset_response.json().get("reset")
    if reset_response.status_code != 200 or not isinstance(reset, Mapping):
        raise RuntimeError("PG-119 fresh reset failed")
    ...
    if not (reset["fresh_target"] and reset["completed"] and reset["evaluator_state_hidden"] and reset["state_change_allowed"] is False):
        raise ValueError("PG-119 reset contract failed")
    payload = _payload(surface, role == "candidate", marker)
    route = f"/v7/observe/{surface.kind}"
    if method == "GET":
    ...
    if response.status_code != 200:
        raise RuntimeError(f"PG-119 target returned {response.status_code}")
    body = response.json()
    ...
    evidence_body = {"schema_version": EVIDENCE_SCHEMA_VERSION, "target_instance_id": f"pg119-metadata-target-{seed}", "surface": surface.kind, "method": method, "role": role, "encoding_chain": ENCODING_CHAIN, "request_sha256": request_hash, "target_evidence_sha256": target_hash, "reset_evidence_sha256": reset_hash, "response_projection": dict(response_projection), "oracle_projection": dict(oracle), "fresh_reset": dict(reset), "safety": {"external_network": False, "script_execution": False, "database_write": False, "state_mutated": False}}
    ...
    action_manifest = {"method": method, "route_template_id": f"pg119-metadata-route-{_hash_text(route)[:16]}", "placement": placement, "encoding_chain": ENCODING_CHAIN, "probe_ref": f"pg119-abstract-triple-probe-{_hash_text(surface.kind)[:12]}", "probe_sha256": request_hash, "safety": {"no_external_network": True, "does_not_execute": True, "no_database_write": True, "no_credential_access": True}}
    if method == "POST":
    ...
    return {"seed": seed, "target_instance_id": f"pg119-metadata-target-{seed}", "surface": surface.kind, "method": method, "role": role, "candidate_signal": bool(oracle["candidate_signal"]), "positive": positive, "positive_authority": authority, "action_manifest": action_manifest, "baseline_projection": model_input["baseline_projection"], "response_projection": response_projection, "model_input": model_input, "oracle_projection": oracle, "fresh_reset": {**dict(reset), "reset_evidence_sha256": reset_hash}, "bridge_evidence": bridge_evidence}
...
async def collect_episode(client: httpx.AsyncClient, surface: Surface, *, seed: int) -> dict[str, Any]:
    episode_id = f"pg119-metadata-{seed}-{surface.kind}"
    ...
    marker = f"pg119{seed}{surface.kind[:3]}"
    ...
    rule_ir_slot_binding = {"slot_id": f"pg119-metadata-slot-{_hash_text(surface.kind)[:16]}", "binding_stage": "after_shadow_probe_and_typed_oracle", "decision": final_decision, "evidence_sha256": steps[-1]["evidence_sha256"], "shadow_probe_evidence_sha256": [step["evidence_sha256"] for step in steps], "typed_oracle_available": surface.typed_available, "positive_authority": final_decision == "confirmed_positive", "long_term_memory_write": False}
    return {"episode_id": episode_id, "target_instance_id": f"pg119-metadata-target-{seed}", "target_seed": seed, "surface_kind": surface.kind, "oracle_available": surface.typed_available, "expected_outcome": surface.expected, "encoding_chain": ENCODING_CHAIN, "steps": steps, "evidence_records": [record["bridge_evidence"] for record in records], "episode_report": evaluate_episode(steps), "final_decision": final_decision, "candidate_pair_positive": surface.expected == "positive" and final_decision == "confirmed_positive", "negative_control_pair_clear": all(not record["positive"] for record in controls), "rule_ir_slot_binding": rule_ir_slot_binding, "bsp": {"parameter_sha256_before": before_parameter, "parameter_sha256_after": state.parameter_sha256(), "parameter_unchanged": before_parameter == state.parameter_sha256(), "topology_version": state.topology_version, "state_sha256": state.state_sha256()}}


async def collect_target(seed: int) -> dict[str, Any]:
    from .pg119_metadata_training_target import create_app

    app = create_app(seed)
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://127.0.0.1") as client:
        health = await client.get("/health")
        if health.status_code != 200 or health.json().get("implementation") != "pg119-metadata-independent-target":
            raise RuntimeError("PG-119 health identity failed")
        episodes = [await collect_episode(client, surface, seed=seed) for surface in SURFACES]
    return {"target_seed": seed, "target_instance_id": f"pg119-metadata-target-{seed}", "target_implementation": "pg119-metadata-independent-target", "target_schema_version": health.json().get("schema_version"), "episodes": episodes}
