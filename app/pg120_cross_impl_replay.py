"""PG-120 GET/POST bridge for the independent eta metadata target."""
...
from .pg120_eta_metadata_target import SURFACES as TARGET_SURFACES
from .trace_aligned_dataset import evaluate_episode, sha256_json, validate_trace_step


SCHEMA_VERSION = "pg120-cross-implementation-replay-v1"
EVIDENCE_SCHEMA_VERSION = "pg120-eta-bridge-evidence-v1"
ORACLE_CONTRACT = "pg120-eta-metadata-transition-typed-oracle-v1"
ENCODING_CHAIN = ["url_percent", "unicode_escape", "html_entity"]
...
def _verify_hash(value: Mapping[str, Any]) -> str:
    ...
    if len(declared) != 64 or any(char not in "0123456789abcdef" for char in declared):
        raise ValueError("PG-120 evidence hash is invalid")
    body = dict(value)
    body.pop("evidence_hash", None)
    if sha256_json(body) != declared:
        raise ValueError("PG-120 evidence hash mismatch")
    return declared


def _payload(surface: Surface, candidate: bool, marker: str) -> dict[str, Any]:
    if surface.kind == "metadata":
        return {"channel_mode": "delta" if candidate else "control"}
    if surface.kind == "decoy":
        return {"layout_mode": "delta" if candidate else "control"}
    if surface.kind == "steady":
        return {"state": "control"}
    return {"phase": "canary" if candidate else "control", "marker": marker}
...
def _oracle(surface: Surface, response: Mapping[str, Any]) -> tuple[dict[str, Any], bool, bool, str]:
    ...
    if not isinstance(target_evidence, Mapping):
        raise ValueError("PG-120 target omitted evidence")
    target_hash = _verify_hash(target_evidence)
    evaluator = target_evidence.get("evaluator")
    if not isinstance(evaluator, Mapping):
        raise ValueError("PG-120 target omitted evaluator")
    ...
    projection = {
        "modality": "metadata_cross_impl_typed_differential" if available else "metadata_cross_impl_untyped_signal",
        "positive": positive,
...
async def _action(client: httpx.AsyncClient, surface: Surface, *, seed: int, decoy_strength: int, method: str, role: str, marker: str, state: BspV3State) -> dict[str, Any]:
    ...
    if health.status_code != 200 or health.json().get("implementation") != "pg120-eta-independent-target":
        raise RuntimeError("PG-120 target identity failed")
    reset_response = await client.post("/v8/reset")
    reset = reset_response.json().get("reset")
    if reset_response.status_code != 200 or not isinstance(reset, Mapping):
        raise RuntimeError("PG-120 fresh reset failed")
    ...
    if not (reset["fresh_target"] and reset["completed"] and reset["evaluator_state_hidden"] and reset["state_change_allowed"] is False):
        raise ValueError("PG-120 reset contract failed")
    payload = _payload(surface, role == "candidate", marker)
    route = f"/v8/probe/{surface.kind}"
    if method == "GET":
    ...
    if response.status_code != 200:
        raise RuntimeError(f"PG-120 target returned {response.status_code}")
    body = response.json()
    ...
    target_id = f"pg120-eta-target-{seed}-d{decoy_strength}"
    ...
    action_manifest = {"method": method, "route_template_id": f"pg120-eta-route-{_hash_text(route)[:16]}", "placement": placement, "encoding_chain": ENCODING_CHAIN, "probe_ref": f"pg120-abstract-triple-probe-{_hash_text(surface.kind)[:12]}", "probe_sha256": request_hash, "safety": {"no_external_network": True, "does_not_execute": True, "no_database_write": True, "no_credential_access": True}}
    if method == "POST":
    ...
    return {"seed": seed, "decoy_strength": decoy_strength, "target_instance_id": target_id, "surface": surface.kind, "method": method, "role": role, "candidate_signal": bool(oracle["candidate_signal"]), "positive": positive, "positive_authority": authority, "action_manifest": action_manifest, "baseline_projection": model_input["baseline_projection"], "response_projection": response_projection, "model_input": model_input, "oracle_projection": oracle, "fresh_reset": {**dict(reset), "reset_evidence_sha256": reset_hash}, "bridge_evidence": bridge_evidence}
...
async def collect_episode(client: httpx.AsyncClient, surface: Surface, *, seed: int, decoy_strength: int) -> dict[str, Any]:
    episode_id = f"pg120-eta-{seed}-d{decoy_strength}-{surface.kind}"
    ...
    marker = f"pg120{seed}{decoy_strength}{surface.kind[:2]}"
