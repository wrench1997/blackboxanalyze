"""Token-aware multitask adapter for PG-203.

Unlike PG-201/202, this adapter receives explicit structural tokens for the
candidate encoding chain and observed failure projection.  These are not
family labels or evaluator answers; they are fields the request controller
already has before choosing the next action.
"""
...
TOKEN_FEATURE_DIM = len(ENCODING_NAMES) + len(FAILURE_NAMES)
TOKEN_SCHEMA = "pg203-token-aware-grounding-adapter-v1"


class TokenAwareGroundingDecoder(nn.Module):
    def __init__(self, frozen_base: nn.Module, d_model: int = 1024, hidden_dim: int = 96) -> None:
        ...
        self.shared = nn.Sequential(nn.Linear(96, hidden_dim), nn.GELU(), nn.LayerNorm(hidden_dim))
    ...
    def forward(self, ids: torch.Tensor, mask: torch.Tensor, features: torch.Tensor, token_features: torch.Tensor) -> dict[str, torch.Tensor]:
        with torch.no_grad():
        ...
        tokens = self.token_projection(token_features)
        shared = self.shared(torch.cat([context, structured, tokens], dim=-1))
        return {
...
def token_features_for_row(row: dict[str, Any]) -> list[float]:
    encoding = int(row.get("encoding_label", 0))
    failure = int(row.get("failure_label", 0))
    if not 0 <= encoding < len(ENCODING_NAMES) or not 0 <= failure < len(FAILURE_NAMES):
        raise ValueError("PG-203 structural token label is out of range")
    return [float(index == encoding) for index in range(len(ENCODING_NAMES))] + [float(index == failure) for index in range(len(FAILURE_NAMES))]


def _tensor_rows(rows: list[dict[str, Any]], device: torch.device) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    tokens = torch.tensor([token_features_for_row(row) for row in rows], dtype=torch.float32, device=device)
    ...
    return features, tokens, actions, encodings, failures
...
def train_token_aware(model: TokenAwareGroundingDecoder, train_rows: list[dict[str, Any]], holdout_rows: list[dict[str, Any]], ids: torch.Tensor, mask: torch.Tensor, *, epochs: int = 80) -> dict[str, Any]:
    if not train_rows or not holdout_rows:
        raise ValueError("PG-203 requires non-empty train and holdout rows")
    device = ids.device
    train_features, train_tokens, train_actions, train_encodings, train_failures = _tensor_rows(train_rows, device)
    holdout_features, holdout_tokens, holdout_actions, holdout_encodings, holdout_failures = _tensor_rows(holdout_rows, device)
    ...
    for epoch in range(1, int(epochs) + 1):
        model.train()
        outputs = model(ids.expand(len(train_rows), -1), mask.expand(len(train_rows), -1), train_features, train_tokens)
        loss = (
        ...
        if epoch in {1, epochs} or epoch % 20 == 0:
            ...
            with torch.inference_mode():
                holdout_outputs = model(ids.expand(len(holdout_rows), -1), mask.expand(len(holdout_rows), -1), holdout_features, holdout_tokens)
    ...
    model.eval()
    with torch.inference_mode():
        train_outputs = model(ids.expand(len(train_rows), -1), mask.expand(len(train_rows), -1), train_features, train_tokens)
        holdout_outputs = model(ids.expand(len(holdout_rows), -1), mask.expand(len(holdout_rows), -1), holdout_features, holdout_tokens)
    return {
...
def evaluate_token_aware(model: TokenAwareGroundingDecoder, rows: list[dict[str, Any]], ids: torch.Tensor, mask: torch.Tensor) -> dict[str, Any]:
    if not rows:
        raise ValueError("PG-203 replay rows must not be empty")
    device = ids.device
    features, tokens, actions, encodings, failures = _tensor_rows(rows, device)
    model.eval()
    with torch.inference_mode():
        outputs = model(ids.expand(len(rows), -1), mask.expand(len(rows), -1), features, tokens)
    return _metrics(outputs, actions, encodings, failures)


def predict_token_aware(model: TokenAwareGroundingDecoder, *, ids: torch.Tensor, mask: torch.Tensor, features: list[float], token_features: list[float]) -> dict[str, Any]:
    if len(token_features) != TOKEN_FEATURE_DIM:
        raise ValueError("PG-203 token feature dimension mismatch")
    ...
    with torch.inference_mode():
        outputs = model(ids, mask, feature_tensor, token_tensor)
...
__all__ = [
    "TOKEN_FEATURE_DIM", "TOKEN_SCHEMA", "TokenAwareGroundingDecoder", "evaluate_token_aware",
    "predict_token_aware", "token_features_for_row", "train_token_aware",
]
