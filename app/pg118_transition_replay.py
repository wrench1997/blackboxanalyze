"""PG-118 local GET/POST replay bridge with a family-free transition slot."""
...
from .pg118_transition_rule_ir_decoder import canonical_model_input
from .pg118_transition_training_target import SURFACES as TARGET_SURFACES
from .trace_aligned_dataset import evaluate_episode, sha256_json, validate_trace_step


SCHEMA_VERSION = "pg118-transition-replay-v1"
EVIDENCE_SCHEMA_VERSION = "pg118-transition-bridge-evidence-v1"
ORACLE_CONTRACT = "pg118-delta-transition-typed-oracle-v1"
ENCODING_CHAIN = ["html_entity", "url_percent"]
...
def _verify_hash(value: Mapping[str, Any]) -> str:
    ...
    if len(declared) != 64 or any(char not in "0123456789abcdef" for char in declared):
        raise ValueError("PG-118 evidence hash is invalid")
    body = dict(value)
    body.pop("evidence_hash", None)
    if sha256_json(body) != declared:
        raise ValueError("PG-118 evidence hash mismatch")
    return declared


def _payload(surface: Surface, candidate: bool, marker: str) -> dict[str, Any]:
    if surface.kind == "route":
        return {"transition_mode": "alternate" if candidate else "baseline"}
...
def _oracle(surface: Surface, response: Mapping[str, Any]) -> tuple[dict[str, Any], bool, bool, str]:
    ...
    if not isinstance(target_evidence, Mapping):
        raise ValueError("PG-118 target omitted evidence")
    target_hash = _verify_hash(target_evidence)
    evaluator = target_evidence.get("evaluator")
    if not isinstance(evaluator, Mapping):
        raise ValueError("PG-118 target omitted evaluator")
    ...
    projection = {
        "modality": "transition_typed_differential" if available else "transition_untyped_signal",
        "positive": positive,
        "positive_authority": positive,
        "candidate_signal": candidate,
        "observed_atoms": ["transition_delta"] if positive else ["candidate_signal_observed"] if candidate else ["no_effect"],
...
async def _action(client: httpx.AsyncClient, surface: Surface, *, seed: int, method: str, role: str, marker: str, state: BspV3State) -> dict[str, Any]:
    ...
    if health.status_code != 200 or health.json().get("implementation") != "pg118-delta-independent-target":
        raise RuntimeError("PG-118 target identity failed")
    reset_response = await client.post("/v6/reset")
    reset = reset_response.json().get("reset")
    if reset_response.status_code != 200 or not isinstance(reset, Mapping):
        raise RuntimeError("PG-118 fresh reset failed")
    ...
    if not (reset["fresh_target"] and reset["completed"] and reset["evaluator_state_hidden"] and reset["state_change_allowed"] is False):
        raise ValueError("PG-118 reset contract failed")
    payload = _payload(surface, role == "candidate", marker)
    route = f"/v6/observe/{surface.kind}"
    if method == "GET":
    ...
    if response.status_code != 200:
        raise RuntimeError(f"PG-118 target returned {response.status_code}")
    body = response.json()
    ...
    evidence_body = {"schema_version": EVIDENCE_SCHEMA_VERSION, "target_instance_id": f"pg118-delta-target-{seed}", "surface": surface.kind, "method": method, "role": role, "encoding_chain": ENCODING_CHAIN, "request_sha256": request_hash, "target_evidence_sha256": target_hash, "reset_evidence_sha256": reset_hash, "response_projection": dict(response_projection), "oracle_projection": dict(oracle), "fresh_reset": dict(reset), "safety": {"external_network": False, "script_execution": False, "database_write": False, "state_mutated": False}}
    ...
    action_manifest = {"method": method, "route_template_id": f"pg118-route-{_hash_text(route)[:16]}", "placement": placement, "encoding_chain": ENCODING_CHAIN, "probe_ref": f"pg118-abstract-double-probe-{_hash_text(surface.kind)[:12]}", "probe_sha256": request_hash, "safety": {"no_external_network": True, "does_not_execute": True, "no_database_write": True, "no_credential_access": True}}
    if method == "POST":
    ...
    return {"seed": seed, "target_instance_id": f"pg118-delta-target-{seed}", "surface": surface.kind, "method": method, "role": role, "candidate_signal": bool(oracle["candidate_signal"]), "positive": positive, "positive_authority": authority, "action_manifest": action_manifest, "baseline_projection": model_input["baseline_projection"], "response_projection": response_projection, "model_input": model_input, "oracle_projection": oracle, "fresh_reset": {**dict(reset), "reset_evidence_sha256": reset_hash}, "bridge_evidence": bridge_evidence}
...
async def collect_episode(client: httpx.AsyncClient, surface: Surface, *, seed: int) -> dict[str, Any]:
    episode_id = f"pg118-delta-{seed}-{surface.kind}"
    ...
    marker = f"pg118{seed}{surface.kind[:3]}"
