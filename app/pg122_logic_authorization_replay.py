"""PG-122 GET/POST replay for an independent authorization-transition target.

This bridge is evaluator-only.  It stores bounded projections and hashes, not
raw probes, response bodies, credentials, or exploit strings.  The trace is
deliberately sequential: each step carries a failure signature that selects a
bounded next replay or a fail-closed abstention.
"""
...
from .pg121_shape_sanitized_rule_ir_decoder import canonical_model_input
from .pg122_logic_authorization_target import SURFACES as TARGET_SURFACES
from .trace_aligned_dataset import evaluate_episode, sha256_json, validate_trace_step


SCHEMA_VERSION = "pg122-authorization-replay-v1"
EVIDENCE_SCHEMA_VERSION = "pg122-authorization-bridge-evidence-v1"
ORACLE_CONTRACT = "pg122-authorization-transition-typed-oracle-v1"
ENCODING_CHAIN = ["html_entity", "url_percent", "unicode_escape"]
...
SURFACES = tuple(
    Surface(kind, "positive" if index == 0 else "negative" if index < 3 else "abstain", index < 3)
    for index, kind in enumerate(TARGET_SURFACES)
)
...
def _verify_hash(value: Mapping[str, Any]) -> str:
    ...
    if len(declared) != 64 or any(char not in "0123456789abcdef" for char in declared):
        raise ValueError("PG-122 evidence hash is invalid")
    body = dict(value)
    body.pop("evidence_hash", None)
    if sha256_json(body) != declared:
        raise ValueError("PG-122 evidence hash mismatch")
    return declared


def _payload(surface: Surface, candidate: bool, marker: str) -> dict[str, Any]:
    if surface.kind == "authorization":
        return {"access_mode": "elevated" if candidate else "normal"}
    if surface.kind == "decoy":
        return {"layout_mode": "elevated" if candidate else "normal"}
...
def _oracle(surface: Surface, response: Mapping[str, Any]) -> tuple[dict[str, Any], bool, bool, str]:
    ...
    if not isinstance(target_evidence, Mapping):
        raise ValueError("PG-122 target omitted evidence")
    target_hash = _verify_hash(target_evidence)
    evaluator = target_evidence.get("evaluator")
    if not isinstance(evaluator, Mapping):
        raise ValueError("PG-122 target omitted evaluator")
    ...
    projection = {
        "modality": "authorization_typed_differential" if available else "authorization_untyped_signal",
        "positive": positive,
        "positive_authority": positive,
        "candidate_signal": candidate,
        "observed_atoms": ["authorization_transition"] if positive else ["candidate_signal_observed"] if candidate else ["no_effect"],
...
async def _action(client: httpx.AsyncClient, surface: Surface, *, seed: int, decoy_strength: int, method: str, role: str, marker: str, state: BspV3State) -> dict[str, Any]:
    ...
    if health.status_code != 200 or health.json().get("implementation") != "pg122-theta-authorization-target":
        raise RuntimeError("PG-122 target identity failed")
    reset_response = await client.post("/v9/reset")
    reset = reset_response.json().get("reset")
    if reset_response.status_code != 200 or not isinstance(reset, Mapping):
        raise RuntimeError("PG-122 fresh reset failed")
    ...
    if not (reset["fresh_target"] and reset["completed"] and reset["evaluator_state_hidden"] and reset["state_change_allowed"] is False):
        raise ValueError("PG-122 reset contract failed")
    payload = _payload(surface, role == "candidate", marker)
    route = f"/v9/check/{surface.kind}"
    if method == "GET":
    ...
    if response.status_code != 200:
        raise RuntimeError(f"PG-122 target returned {response.status_code}")
    body = response.json()
    ...
    target_id = f"pg122-theta-target-{seed}-d{decoy_strength}"
    ...
    action_manifest = {"method": method, "route_template_id": f"pg122-theta-route-{_hash_text(route)[:16]}", "placement": placement, "encoding_chain": ENCODING_CHAIN, "probe_ref": f"pg122-abstract-triple-probe-{_hash_text(surface.kind)[:12]}", "probe_sha256": request_hash, "safety": {"no_external_network": True, "does_not_execute": True, "no_database_write": True, "no_credential_access": True}}
    if method == "POST":
...
def _step(record: Mapping[str, Any], *, episode_id: str, index: int, parent: str | None, decision: str, next_action: str, before: Mapping[str, float], after: Mapping[str, float], pair_id: str | None, prior_records: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    # The scheduler is authoritative for the transition; a final positive is
    # still gated by the collector's typed oracle and matched pair.
    if failure["kind"] == "typed_positive" and decision != "confirmed_positive":
    ...
    step = validate_trace_step({"episode_id": episode_id, "step_id": f"{episode_id}-s{index:02d}", "parent_step_id": parent, "sampling_seed": record["seed"], "target_instance_id": record["target_instance_id"], "hypothesis": "unknown_authorization_surface", **body, "fresh_reset": record["fresh_reset"], "evidence_sha256": record["bridge_evidence"]["evidence_hash"], "dataset_stage": "pg122_failure_guided_authorization_holdout", "online_weight_update": False, "long_term_memory_write": False, "echo": {"sha256": sha256_json(body)}})
...
async def collect_episode(client: httpx.AsyncClient, surface: Surface, *, seed: int, decoy_strength: int) -> dict[str, Any]:
    episode_id = f"pg122-theta-{seed}-d{decoy_strength}-{surface.kind}"
    ...
    marker = f"pg122{seed}{decoy_strength}{surface.kind[:2]}"
